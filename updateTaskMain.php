...
include ("class/sql_getupdateTaskMain.php"); //  sql commands for get UpdatetaskMain Details
...
<script>
		...
		function selectUser(){
			...
			var ff = frm_porject.lstSysUsers.value;
           var tt = document.getElementById('txtTaskCode').value;
		    var uu = document.getElementById('loguser123').value;
		   //alert (ff);
            $.post('class/sql_getupdateTaskMain.php',{selectUser :ff , task :tt , loguser :uu},
                function(output){                    
		...
		function removeUser(){   
		var tt = document.getElementById('txtTaskCode').value;         
            $.post('class/sql_getupdateTaskMain.php',{removeUser : frm_porject.lstFacUsers.value, task:tt},
                function(output){                    
                    $('#lstFacUsers').html(output);   
                    refreshList();
                }
...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready(function() {
		...
		$('#optPriority').change(function(){
			var theVal = $('#optPriority').val();
			switch(theVal){
   			 case'Task Started':
				$('#lstSysUsers').prop('disabled', true);
				$('#lstFacUsers').prop('disabled', true);
			...
			case'Task Completed':
				...
				break;
			case'Impediment':
				$('#lstSysUsers').prop('disabled', false);
				$('#lstFacUsers').prop('disabled', false);
...
<script type="text/javascript">
    ...
    function startUpload(){
        var valdator = false;
		var ss = $('#optPriority').val();
		aa = $('#lstFacUsers option:first-child').val();
        valdator = $("#frm_porject").valid();
		if(($('#optPriority').val() == "Impediment")&&(aa == null) ){
					//alert (ss+"  "+aa);
					document.getElementById('error').innerHTML = '*** Please Select the Responsible Person ***';
					return false;
		}
       else{		
			if(valdator != false){
    ...
    <?php
	 if(isset($_GET["taskcode"]))
        ...
        if(isset($_GET["taskcode"]))
        {
			...
			$_FAC = $Str_TaskCode;
			$_FacilitySet = getTASKUSERS123($str_dbconnect,$_FAC);
            ...
            while($_myrowRes = mysqli_fetch_array($_ResultSet)) {
                ...
                $MailCCTo       =   $_myrowRes['MailCCTo'];
                //$Str_TaskDescription    =   $_myrowRes['TaskDetails'];
				$Str_UserTaskDescription = $_myrowRes['TaskDetails'];
            }
...
//                $Str_ProCode    = create_project($str_dbconnect,$_POST["txtProName"], $_POST["txt_StartDate"]);
               ...
               // $mailer->IsSMTP();
                    // $mailer->Host = '10.9.0.166:25'; //$mailer->Host = 'ssl://smtp.gmail.com:465';   // $mailer->Host = '10.9.0.165:25';
                    // $mailer->SetLanguage("en", 'class/');				// $mailer->SetLanguage("en", 'class/');
                    // $mailer->SMTPAuth = TRUE;
                    // $mailer->IsHTML = TRUE;
                    // $mailer->Username = 'pms@eTeKnowledge.com';  // Change this to your gmail adress  info@tropicalfishofasia.com
                    // $mailer->Password = 'pms@321';  // Change this to your gmail password   info321
                ...
                $MailTitile =   "FROM : ".getSELECTEDEMPLOYENAME($str_dbconnect,$_SESSION["LogEmpCode"])." - TO : ".getSELECTEDEMPLOYEFIRSTNAMEONLY($str_dbconnect,$_ProInit)." - TASK UPDATE- ".$_Division." ".$_Department." - ".$Str_TaskName;
                $mailer->Subject = $MailTitile;

                $mailer->AddAddress($StrToMail);  // This is where you put the email adress of the person you want to mail  - $StrToMail
       ...
       if($_POST["optPriority"] == "Impediment"){

                    $MagBody        =   CreateMailImpediment($str_dbconnect,$Str_ProCode, $Str_ProName, $Str_TaskCode, $Str_TaskName, $_POST["optPriority"], $_POST["txtTaskDescription"], $Dte_SysDate);
                    // $mailer = new PHPMailer();
                    // $mailer->IsSMTP();
                    // $mailer->Host = '10.9.0.166:25';				// $mailer->Host = 'ssl://smtp.gmail.com:465';
                    // $mailer->SetLanguage("en", 'class/');					// $mailer->SetLanguage("en", 'class/');
                    ...
                    // $mailer->Username = 'pms@eTeKnowledge.com';  // Change this to your gmail adress    $mailer->Username = 'info@tropicalfishofasia.com';
                    // $mailer->Password = 'pms@321';  // Change this to your gmail password   $mailer->Password = 'info321'; 
                    // $mailer->From = $StrFromMail;  // This HAVE TO be your gmail adress      $StrFromMail
                    // $mailer->FromName = 'PMS - '.$StrSenderName; // This is the from name in the email, you can put anything you like here
                    // $mailer->Body = $MagBody;
					//O365 Email Function Start
					$mailer = new PHPMailer();
                    // var_dump('nf locally'); die();
					$mailer->IsSMTP();
					$mailer->Host = 'smtp.office365.com';
					$mailer->SetLanguage("en", 'class/');					
					$mailer->SMTPAuth = TRUE;
                    ...
                    $MailTitile =   "TO : ".getSELECTEDEMPLOYEFIRSTNAMEIMPEDIMENT($str_dbconnect,$Str_TaskCode)." - Project Impediment Alert- ".$_Division." ".$_Department." - ".$Str_TaskName;
                    $mailer->Subject = $MailTitile;
					// $mailer->AddAddress($StrToMail);
                    $mailer->AddAddress($StrToMail); // This is where you put the email adress of the person you want to mail

					//$impemail = getImpedimentEMPLOYEESMAIL($str_dbconnect,$Str_TaskCode);
		...
		$_SelectQuery 	=   "SELECT EmpCode FROM tbl_impedimenttask WHERE `TaskCode` = '$Str_TaskCode' and im_status='P'" or die(mysqli_error($str_dbconnect));
					...
					while($_myrowRes = mysqli_fetch_array($_ResultSet)) {						
						...
						$loggedUser = $_myrowRes['OwnerEmpCode'];
							...
							while($_myrowRes2 = mysqli_fetch_array($_ResultSet2)) 
							{
								$mailer->AddBCC($_myrowRes2['EMail']);
							}
						}						 
					}
					/*Adding Bcc Function on 2014-07-16 by thilina*/
				...
				//O365 Email Function END			
                    ...
                    $MailTitile =   "FROM : ".getSELECTEDEMPLOYENAME($str_dbconnect,$_SESSION["LogEmpCode"])." - TO : ".getSELECTEDEMPLOYEFIRSTNAMEONLY($str_dbconnect,$_ProInit)." - APPROVAL FOR TASK COMPLETION - ".$_Division." ".$_Department." - ".$Str_TaskName;
                    $mailer->Subject = $MailTitile;
            ...
            echo "<script>";
            echo "self.location='updateTaskMain.php?&taskcode=$Str_TaskCode ';";
            echo "</script>";
...
//http://localhost/test1/PMS/updateTaskMain.php?&taskcode=TSK/4700
    ?>
                        ...
                        <td align="left" style="width: 100%; vertical-align: top;">
                            <div id="right" >
                                <table width="100%" cellpadding="0" cellspacing="0">
                                    <tr style="height: 50px; background-color: #E0E0E0;">
                                        <td style="padding-left: 10px; font-size: 16px">
                                            <font color="#0066FF">Task Update</font>                                                
                                        </td>                                            
                                    </tr>    
                                    <tr align="center">
                                ...
                                <table width="98%" cellpadding="0" cellspacing="0" align="center" style="color: #000000">
                                    <tr>
                                        <td >                                                
                                            <input type="submit"  id="btnBack" name="btnBack" title="Go to Previous Page" value="Back to Previous Screen" size="5"/> <input type="hidden" id="loguser123" name="loguser123" value="<?php echo $_SESSION["LogEmpCode"]; ?>">
                                            <table width="100%" cellpadding="0" cellspacing="8px">
												...
												<tr>
                                                    ...
                                                    <td>:</td>
                                                    <td>
                                                        <textarea cols="80" rows="5" style="width: 100%" id="txtUserTaskDescription" name="txtUserTaskDescription" class="tinymce" <?php if($bool_ReadOnly == "TRUE") echo "readonly=\"readonly\";" ?>><?php echo $Str_UserTaskDescription;?></textarea>
                                                    </td>
                                                </tr>												
                                                <tr>
                                                    <td>Category</td>
                                                    <td></td>
                                                    <td>
                                                        <select id="optPriority" name="optPriority" class="TextBoxStyle" <?php if($bool_ReadOnly == "TRUE") echo "disabled=\"disabled\";" ?> onChange="categoryChange()">
                                                            <?php
                                                            $IneerCount =   0;
                                                            ...
                                                            ?>
																...
																<option id="Suspend due to N/A" value="Suspend due to N/A" >Suspend due to N/A &nbsp;&nbsp;</option>
																<option id="No longer Task Owner" value="No longer Task Owner" >No longer Task Owner &nbsp;&nbsp;</option>
                                                ...
                                                <div class="slider" style="top: 360px; left: 24%; width: 200px"></div>
                                                ...
                                                <tr>
                                                  ...
                                                  <tr>
                                                    <td width="20%">
                                                        Resposible Persons for the Impediment
                                                    </td>
                                                    <td width="2%">:</td>
                                                    <td>
                                                        <select name="lstSysUsers" size="10" class="" id="lstSysUsers" style="width:200px" <?php if ($bool_ReadOnly == "TRUE") echo "disabled=\"disabled\";" ?> disabled>
										                    <?php
										                    ...
										                    <input name="Save" type="button"  id="Save" value=">" onClick="selectUser()"  style="width: 40px; vertical-align: 500%; cursor: pointer" <?php if ($Save_Enable == "No") echo "disabled=\"disabled\";" ?> disabled/>
										                    <input name="Del" type="button"  id="Del" value="<"  style="width: 40px; vertical-align: 500%; cursor: pointer" <?php if ($Save_Enable == "No") echo "disabled=\"disabled\";" ?> disabled onclick="removeUser()" />
										
										                    <select name="lstFacUsers" size="10" class="" id="lstFacUsers" onclick="removeUser() style="width:200px"  disabled>
										                   <option> </option>
										                 </select> <span class="error" id="error">* <?php echo $nameErr;?> </span>
                                                    </td>
                                                </tr>
                                                            ...
                                                            <tr>
                                                                <td align="left" style="vertical-align: text-top">
                                                                    <p>
                                                                        <?php
                                                                            $ItemCount = 0;
                                                                            $_ResultSet      = get_projectupload($str_dbconnect,$DocId) ;
                                                                            while($_myrowRes = mysqli_fetch_array($_ResultSet)) {
                                                                                $ItemCount += 1;
                                                    ...
                                                    ?>
                                                        <tr class="<?php if($UpdateUser != $_EmpCode && $UpdateUser != "") {echo $ClassS;} else {echo $Class;} ?>" >
                                                            ...
                                                            <td><?php echo $_myrowRes['Note']; ?></td>
                                                             <td><?php echo $_myrowRes['up_status']; ?></td>
                                                            <td align="center">
                                                                <?php
                                                                    echo $_myrowRes['UpdateDate'];
															...
															<td align="center">
                                                                <?php
																	if($_myrowRes['UpdateUser'] == ""){
																		...
																		echo get_empname($str_dbconnect,$_myrowRes['UpdateUser']);//                                                                   
																	}
                                                                ?>
                                                            </td>
                                                        </tr>
