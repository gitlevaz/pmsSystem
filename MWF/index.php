	...
	<head>	
		...
		<script type="text/javascript">
			...
			function LoadStations(MCID,MCNAME){				
				window.location = "StationUpdate.php?MCID="+MCID+"&MCNAME="+MCNAME;			
			}
			
			function USInventory(MCID,MCNAME){				
				window.location = "USInventory.php?MCID="+MCID+"&MCNAME="+MCNAME;			
			}
	...
	if($Country == "CN"){
		...
		if($Country == "FIJI"){
			...
			{			
				...
				/*if(isset($_POST["txt_MCID"])){*/
					$MCID =	$_POST["txt_MCID"]; 		
				/*}*/
				
				/*if(isset($_POST["txt_MCNAME"])){*/
					$MCNAME =	$_POST["txt_MCNAME"]; 		
				/*}*/
				...
				if ($str_Authonticate != "-") {
					...
					if($MCID != ""){
						if(substr($MCID, 0, 1) == "M"){											
							echo "<script language='javascript' type='text/javascript'>LoadServices('".$MCID."','".$MCNAME."');</script>";						
						}else if(substr($MCID, 0, 1) == "D"){
							echo "<script language='javascript' type='text/javascript'>USInventory('".$MCID."','".$MCNAME."');</script>";						
						}else{							
			...
			<div data-role="content">						
				<fieldset data-role="controlgroup" style="text-align:center;">
		            ...
		            <label for="txt_userID" >
		            	...
		            	<input name="txt_userID" id="txt_userID" placeholder="" value="" type="password" pattern="[0-9]*" />
						<input name="txt_MCID" id="txt_MCID"  value="<?php echo $MCID; ?>" type="hidden" />
						<input name="txt_MCNAME" id="txt_MCNAME"  value="<?php echo $MCNAME; ?>" type="hidden" />
						<br/>				
						<br/>				
