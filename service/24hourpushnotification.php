...
//$connection = include_once('../connection/previewconnection.php');
 ...
 while($r = mysqli_fetch_assoc($Result))
 {
  ...
  foreach ($rows as $value) 
  {
    ...
    foreach ($rows as $val) 
     {
		 $currentDate=Date('m-d-Y', strtotime("+1 days"));
        ...
        if($currentDate == $val["taskEndDate"] )
        {
			...
			$query = "SELECT pushNotificationDeviceId,DeviceId FROM tbl_pushnotificationdevice WHERE EmployeeId='".$value["EmployeeId"]."'";        
			$Result=mysqli_query($link,$query) or die(mysqli_error($link));
...
//echo 'd';
				...
				$Insert = "INSERT INTO tbl_pushnotificationmessage (EmpCode, Message ,Category,Url,Title,SubTitle) VALUES ('$empcode', '$Message','24hr format','$url','$Title','$SubTitle')";                 
