...
$totIncompPrstg = 0;
        ...
        $HTML0 .= "</head>";
        $HTML0 .= "<body>";		
		$HTML0 .= "<h3 align=\"center\" style=\"font-family:calibri;\">US-Workflow: Status of workflow for $show_date</h3>";
        ...
        $departmentSql = "SELECT `GrpCode`, `Group` FROM `tbl_projectgroups` WHERE Country = 'US' ORDER BY `Group` ASC";
		...
		while($departmentRow = mysqli_fetch_assoc($departmentResult)) {
			...
			while($ownerRow = mysqli_fetch_assoc($ownerResult)) {				
                ...
                if($dueNo['dueNo']!=0)
				{
					...
					$HTML0 .= "<td align='center' style=\"width:60px;font-size:13px;font-family:Arial, Helvetica, sans-serif\" >";
					$HTML0 .= $ownerRow['FirstName'] . " " . $ownerRow['LastName'];
					$HTML0 .= "</td>";
				
					if($status == "Pending")
					{
						$HTML0 .= "<td bgcolor='#FE2E2E'align='center' style=\"width:60px;font-size:13px;font-family:Arial, Helvetica, sans-serif\" >";
						$HTML0 .= $status;
						$HTML0 .= "</td>";		
					}
					else if($status == "Completed")
					{
						$HTML0 .= "<td bgcolor='#2EFE2E' align='center' style=\"width:60px;font-size:13px;font-family:Arial, Helvetica, sans-serif\" >";
						$HTML0 .= $status;
						$HTML0 .= "</td>";		
					}
