...
//$connection = include_once('../connection/sqlconnection.php');
 ...
 while($r = mysqli_fetch_assoc($Result))
 {
  ...
  foreach ($rows as $value) 
  {
    ...
    foreach ($rows as $val) 
     {
		 $currentDate=Date('m-d-Y', strtotime("+3 days"));
        ...
        if($currentDate == $val["taskEndDate"] )
        {
			...
			if($json->success == 1)			
			{
				...
				$Insert = "INSERT INTO tbl_pushnotificationmessage (EmpCode, Message,Category,Url,Title,SubTitle) VALUES ('$empcode', '$Message','72hr format','$url','$Title','$SubTitle')";               
