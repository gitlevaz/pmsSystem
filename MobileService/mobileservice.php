<?php
    ...
    mysqli_select_db($str_dbconnect,"$str_Database") or die("Unable to establish connection to the MySql database");
		...
		if(isset($_GET['Command']) && $_GET['Command'] == "ActiveProjectsByTaskOwner" && isset($_GET['employeeCode'])){
			...
			$str_SelectQuery    = 	"SELECT * FROM tbl_projects WHERE active = 1 AND procode IN (SELECT DISTINCT procode FROM tbl_task WHERE taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$_employeeCode'))" or die(mysqli_error($str_dbconnect));
			$str_ResultSet     =   mysqli_query($str_dbconnect,$str_SelectQuery) or die(mysqli_error($str_dbconnect));
		...
		if(isset($_GET['Command']) && $_GET['Command'] == "TaskListUnderProjectByTaskOwner" && isset($_GET['employeeCode']) && isset($_GET['projectCode'])){
			...
			$str_SelectQuery    = 	"SELECT * FROM tbl_task WHERE active = 1 AND procode = '$_projectCode' AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$_employeeCode') ORDER BY sublevel, parent" or die(mysqli_error($str_dbconnect));
			$str_ResultSet     =   mysqli_query($str_dbconnect,$str_SelectQuery) or die(mysqli_error($str_dbconnect));
