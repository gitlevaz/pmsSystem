...
function get_TaskDetails($str_dbconnect,$Str_ProjectCode) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND procode = '$Str_ProjectCode' ORDER BY sublevel, parent" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_TaskPrimaryUpdate($str_dbconnect,$Str_TaskCode, $StrCategoryCode) {
    ...
    $_SelectQuery       = 	    "SELECT * FROM tbl_task WHERE active = 1 AND procode = '$Str_ProCode'" or die(mysqli_error($str_dbconnect));
    ...
    $_SelectQuery       = 	    "SELECT * FROM tbl_task WHERE active = 1 AND procode = '$Str_ProCode' AND `taskstatus` <> 'C' AND `taskstatus` <> 'W'" or die(mysqli_error($str_dbconnect));
    $RecordCount        =       mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
    $num_AllActive      =       mysqli_num_rows($RecordCount);

    $_SelectQuery       = 	    "SELECT * FROM tbl_task WHERE active = 1 AND procode = '$Str_ProCode' AND `taskstatus` = 'C'" or die(mysqli_error($str_dbconnect));
    $RecordCount        =       mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
    $num_AllCompleted   =       mysqli_num_rows($RecordCount);

    $_SelectQuery       = 	    "SELECT * FROM tbl_task WHERE active = 1 AND procode = '$Str_ProCode' AND `taskstatus` = 'W'" or die(mysqli_error($str_dbconnect));
    $RecordCount        =       mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_TaskDetailsEmp($str_dbconnect,$Str_ProjectCode, $EmpCode) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND procode = '$Str_ProjectCode' AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EmpCode') ORDER BY sublevel, parent" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_TaskDetailsEmpPRINT($str_dbconnect,$Str_ProjectCode, $EmpCode, $_TskStatus) {
    ...
    if($EmpCode != "ALL"){
        if($_TskStatus != "ALL"){
            $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskstatus = '$_TskStatus' AND procode = '$Str_ProjectCode' AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EmpCode') ORDER BY sublevel, parent" or die(mysqli_error($str_dbconnect));
        }else{
            $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND procode = '$Str_ProjectCode'  AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EmpCode') ORDER BY sublevel, parent" or die(mysqli_error($str_dbconnect));
        }
    }else{
        if($_TskStatus != "ALL"){
            $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskstatus = '$_TskStatus' AND procode = '$Str_ProjectCode' ORDER BY sublevel, parent" or die(mysqli_error($str_dbconnect));
        }else{
            $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND procode = '$Str_ProjectCode' ORDER BY sublevel, parent" or die(mysqli_error($str_dbconnect));
        }
    }
...
function get_TaskSubLevel($str_dbconnect,$Str_TaskCode) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskcode = '$Str_TaskCode' ORDER BY sublevel, parent" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_TaskDetailsParent($str_dbconnect,$Str_taskCode, $Str_Sublevel) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND parent = '$Str_taskCode' AND sublevel = '$Str_Sublevel' ORDER BY parent" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function create_Task($str_dbconnect,$Str_ProCode,$Str_ProCat,$Str_TaskName, $Str_TaskDescription, $Str_TaskParent, $Str_StartDate, $Str_EndDate, $Str_EstHours,$Str_Priority, $MailCC,$kjrcode,$indicaotrcode,$subindicatorcode,$ttcode,$filecount){
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskcode = '$Str_TaskParent' ORDER BY sublevel, parent" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
    while($_myrowRes = mysqli_fetch_array($_ResultSet)) {
    ...
    if($MailCC <> "") {
	 ...
	 list($allhours,$mins,$secs)= explode(':', $Str_EstHours);

    $Str_EstHours = $Str_EstHours."00:00";
	$assi="";
	 //echo $epfno;

    $_SelectQuery   = 	"INSERT INTO tbl_task (`compcode`, `procode`, `taskcode`,`procat`, `parent`, `sublevel`, `taskname`, `TaskDetails`, `taskcrtdate`, `taskenddate`, `AllHours`, `assignuser`, `Priority`, `taskstatus`, `AssignBy`, `MailCCTo`,`KJRid`,`Indicatorid`,`SubIndicatorid`,`Doc_id`)VALUES ('$_CompCode', '$Str_ProCode', '$Str_TaskCode','$Str_ProCat', '$Str_TaskParent', '$Str_SubLevel', '$Str_TaskName', '$Str_TaskDescription',  '$Str_StartDate', '$Str_EndDate', '$allhours', '', '$Str_Priority',  'I', '$_CrtBy', '$Str_MailAddress','$kjrcode','$indicaotrcode','$subindicatorcode','$DocId')" or die(mysqli_error($str_dbconnect));
    mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function UpdateTASKDETAILSKJRBase($str_dbconnect,$ttcode){
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskcode = '$ttcode'" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
    while($_myrowRes = mysqli_fetch_array($_ResultSet)) {
...
function DeleteMain_Task($str_dbconnect,$Str_TaskCode){
    ...
    $_SelectQuery   = 	"UPDATE  tbl_task  SET `active` = 0 WHERE `taskcode` = '$Str_TaskCode'" or die(mysqli_error($str_dbconnect));
    mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));

    $_SelectQuery   = 	"UPDATE prodocumets  SET `active` = 0 WHERE  `ParaCode` = '$Str_TaskCode'";
    mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));

    $_SelectQuery   = 	"UPDATE tbl_taskowners  SET `active` = 0 WHERE   `TaskCode` = '$Str_TaskCode'";
    mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
/*
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$_EmpCode') ORDER BY `procode`" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
/*
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE  active = 1 AND taskstatus <> 'C' AND procode = '$ProCode'  ORDER BY `procode`" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
/*
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskstatus <> 'C'  AND procode = '$ProCode' AND taskcode
    IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$_EmpCode') ORDER BY `procode`" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function updateTaskStatus($str_dbconnect,$_TaskCode, $_Priority, $_TaskDescription, $Precentage, $Start, $End, $HoursSpent, $HrsRequest){
    ...
    switch ($_Priority) {
        ...
        case'Approval':
             $taskstatus = 'W';
            break;
        case'Addl Hrs Request':
            //  $taskstatus = 'A';
            break;
        case'Suspend due to N/A':
             $taskstatus = 'S';
            break;
        case'On-Hold':
             $taskstatus = 'H';
      ...
      $_SelectQuery   = 	"UPDATE tbl_task SET `taskstatus` = '$taskstatus' WHERE `taskcode` = '$_TaskCode'" or die(mysqli_error($str_dbconnect));
      mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));

      $_SelectQuery       = "SELECT * FROM tbl_task WHERE active = 1 AND taskcode = '$_TaskCode'" or die(mysqli_error($str_dbconnect));
      $RecordCount        =  mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function updateTaskStatusApprove($str_dbconnect,$_TaskCode, $_Priority, $_TaskDescription, $Precentage, $Start, $End, $HoursSpent, $HrsRequest){
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_projects WHERE AND active = 1 procode IN (SELECT procode FROM tbl_task WHERE taskcode = '$_TaskCode')" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
    while($_myrowRes = mysqli_fetch_array($_ResultSet)) {
        $_projeccode    =   $_myrowRes['procode'];
        $_crtusercode   =   $_myrowRes['crtusercode'];
...
function getTotalTaskEstimatedHours($str_dbconnect,$Str_TskCode){
    ...
    $query1 = "SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(`AllHours`))) As total FROM `tbl_task` WHERE `taskcode` = '$Str_TskCode'" or die(mysqli_error($str_dbconnect));
    $result1 = mysqli_query($str_dbconnect,$query1);
    $row = mysqli_fetch_assoc($result1);
    $Str_Spent = $row['total'];
...
function get_NoOfTaskforEmp($str_dbconnect,$EMPCODE){
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskcode in (SELECT taskcode  FROM tbl_taskowners WHERE EmpCode = '$EMPCODE')" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_NoOfTaskforEmpTC($str_dbconnect,$EMPCODE){
    /*TASK COMPLETED*/
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskstatus = 'W' AND taskcode  in (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EMPCODE')" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function  get_NoOfTaskforEmpTI($str_dbconnect,$EMPCODE){
    /*TASK IN PROGRESS*/
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskstatus = 'I' AND taskcode  in (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EMPCODE')" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function   get_NoOfTaskforEmpCW($str_dbconnect,$EMPCODE){
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskstatus = 'C' AND taskcode  in (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EMPCODE')" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function   get_NoOfTaskforEmpTN($str_dbconnect,$EMPCODE){
    /*TASK NOT STARTED */
    $_SelectQuery   = 	"SELECT * FROM tbl_task WHERE active = 1 AND taskstatus = 'A' AND taskcode  in (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EMPCODE')" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
