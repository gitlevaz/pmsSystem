...
//  TO Create Project in the System
...
function get_ProjectDetails($str_dbconnect) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_projects" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_SelectedProjectDetails($str_dbconnect,$Str_ProjectCode) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_projects WHERE procode = '$Str_ProjectCode'" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));

    return $_ResultSet;
}

function update_project($str_dbconnect,$Str_ProCode,$Str_ProCat,$Str_ProName, $Dte_StartDate, $_strEMPCODE, $Dte_Enddate, $_strDptCode, $_strDivCode, $_strProInit, $_strSecOwner, $strSupport){

    $_CompCode      =   $_SESSION["CompCode"];

    $_SelectQuery   = 	"UPDATE tbl_projects SET `procode` = '".$Str_ProCode."',`proCat` = '".$Str_ProCat."',`proname` = '" . mysqli_real_escape_string($str_dbconnect,$Str_ProName) . "', `startdate` = '$Dte_StartDate', `ProOwner` = '$_strEMPCODE', `EndDate` = '$Dte_Enddate', `Department` = '$_strDptCode', `Division` = '$_strDivCode', `ProInit` = '$_strProInit', `SecOwner` = '$_strSecOwner', `Support` = '$strSupport'  WHERE `procode` = '$Str_ProCode' AND CompCode = '$_CompCode'" or die(mysqli_error($str_dbconnect));
    mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_ActiveProjectDetails($str_dbconnect) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_projects WHERE prostatus <> 'C' ORDER BY `procode`" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_ActiveProjectDetailsDepartment($str_dbconnect,$DeptCode) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_projects WHERE `prostatus` <> 'C' AND `Department` = '$DeptCode' ORDER BY `procode`" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_ProjectDetailsTask($str_dbconnect) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_projects WHERE procode IN (SELECT DISTINCT procode FROM tbl_task WHERE taskstatus <> 'C'  AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EMPCodeLog')) AND ProOwner='$EMPCodeLog'" or die(mysqli_error($str_dbconnect));
    //$_SelectQuery   = 	"SELECT * FROM tbl_projects WHERE procode IN (SELECT DISTINCT procode FROM tbl_task WHERE taskstatus <> 'C'  AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EMPCodeLog')) OR ProOwner='$EMPCodeLog'" or die(mysqli_error($str_dbconnect));
...
function get_USERProjectDetailsTask($str_dbconnect,$EMPCodeLog) {
    ...
    //$_SelectQuery   = 	"SELECT * FROM tbl_projects WHERE procode IN (SELECT DISTINCT procode FROM tbl_task WHERE taskstatus <> 'C'  AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EMPCodeLog'))" or die(mysqli_error($str_dbconnect));
    $_SelectQuery   = 	"SELECT * FROM tbl_projects where ProOwner='$EMPCodeLog'";
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function get_ProjectDetailsTaskEmployee($str_dbconnect,$EMPCode) {
    ...
    $_SelectQuery   = 	"SELECT * FROM tbl_projects WHERE procode IN (SELECT DISTINCT procode FROM tbl_task WHERE taskstatus <> 'C'  AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EMPCode'))" or die(mysqli_error($str_dbconnect));
    $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
...
function SetupProOrderSequence($str_dbconnect){
	...
	if($num_rows_Projects > $num_rows_Priority){
		 ...
		 $_SelectQuery   = 	"SELECT * FROM tbl_projects WHERE procode NOT IN (SELECT ProCode FROM tbl_ProPriority WHERE EmpCode = '$EMPCodeLog') AND procode IN (SELECT DISTINCT procode FROM tbl_task WHERE taskstatus <> 'C'  AND taskcode IN (SELECT TaskCode FROM tbl_taskowners WHERE EmpCode = '$EMPCodeLog'))" or die(mysqli_error($str_dbconnect));
	     $_ResultSet     =   mysqli_query($str_dbconnect,$_SelectQuery) or die(mysqli_error($str_dbconnect));
